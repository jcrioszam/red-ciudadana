...
import { Surface, Text, ActivityIndicator, Card, Button, Chip, FAB } from 'react-native-paper';
...
export default function DashboardCiudadanosScreen() {
  ...
  // Función para calcular el centro del mapa basado en los reportes
  const calculateMapCenter = (reportes: any[]) => {
    ...
    const validReportes = reportes.filter(r => r.latitud && r.longitud);
    ...
    const latitudes = validReportes.map(r => r.latitud);
    const longitudes = validReportes.map(r => r.longitud);

    const centerLat = latitudes.reduce((a, b) => a + b) / latitudes.length;
    const centerLng = longitudes.reduce((a, b) => a + b) / longitudes.length;

    // Calcular delta para el zoom
  ...
  if (loading) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>Cargando información ciudadana...</Text>
      </View>
    );
  }
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Panel Ciudadano</Text>
        </View>
        ...
        <Surface style={styles.card} elevation={4}>
          <View style={styles.cardHeader}>
            ...
            <Text style={styles.cardTitle}>Reportes en Tiempo Real</Text>
          </View>
          ...
          <View style={styles.statsGrid}>
            ...
            <View style={[styles.statCard, { backgroundColor: '#FFEBEE' }]}>
              ...
              <Text style={styles.statNumber}>{stats.urgentes}</Text>
        ...
        <Surface style={styles.card} elevation={4}>
          <View style={styles.cardHeader}>
            ...
            <Text style={styles.cardTitle}>Mapa en Vivo</Text>
            <Button 
              mode="text" 
        ...
        <Surface style={styles.card} elevation={4}>
          ...
          {reportesData.length === 0 ? (
            ...
            reportesData.slice(0, 5).map((reporte: any, index: number) => (
        ...
        <Surface style={styles.card} elevation={4}>
          <View style={styles.cardHeader}>
            <MaterialIcons name="newspaper" size={24} color="#1a237e" />
            <Text style={styles.cardTitle}>Noticias Recientes</Text>
          </View>
          
          {noticiasData.length === 0 ? (
            <View style={styles.noDataContainer}>
              <MaterialIcons name="article" size={48} color="#ccc" />
              <Text style={styles.noData}>No hay noticias disponibles</Text>
            </View>
          ) : (
            noticiasData.slice(0, 3).map((noticia: any, index: number) => (
              <Card key={noticia.id} style={styles.noticiaCard}>
                <Card.Content>
                  <View style={styles.noticiaHeader}>
                    <Text style={styles.noticiaTitulo}>{noticia.titulo}</Text>
                    <Text style={styles.noticiaFecha}>{formatDate(noticia.fecha_publicacion)}</Text>
                  </View>
                  
                  <Text style={styles.noticiaDescripcion}>{noticia.contenido}</Text>
                  
                  {noticia.autor && (
                    <Text style={styles.noticiaAutor}>
                      Por: {noticia.autor}
                    </Text>
                  )}
                </Card.Content>
